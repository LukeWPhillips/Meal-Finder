...
function getIngredients(e) {
    ...
    if (input.trim()) {
        fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${input}`)
            ...
            .then(data => {
                console.log(data)
                resultHeading.innerHTML = `<h2> Meals containing "${input}"</h2>`
                if (data.meals === null) {
                    resultHeading.innerHTML = `<h2> Try Again!</h2>`
                } else {
...
ingredient.addEventListener('submit', getIngredients)
